...
const ApplicantsTable = () => {
  ...
  const filteredApplicants = useMemo(() => {
    ...
    if (statusFilter !== "All") {
      data = data.filter(
        (item) => (updatedStatus[item._id] || item.status) === statusFilter
      );
    }

    data = [...data].sort((a, b) =>
      sortOrder === "new"
        ? new Date(b.applicant.createdAt) - new Date(a.applicant.createdAt)
        : new Date(a.applicant.createdAt) - new Date(b.applicant.createdAt)
    );
  ...
  return (
    <div className="space-y-4 sm:space-y-5 lg:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 sm:justify-end">
        ...
        <div className="relative w-full sm:w-60">
          ...
          <input
            ...
            className="border rounded-xl pl-9 pr-3 py-2 text-sm w-full focus:outline-none focus:ring-2 focus:ring-[#6A38C2] focus:ring-offset-1"
          />
        </div>
        ...
        <div className="flex gap-2 sm:gap-3">
          <select
            ...
            className="border rounded-xl px-3 py-2 text-xs sm:text-sm flex-1 sm:flex-none"
          >
            <option>All</option>
            <option>Accepted</option>
            <option>Rejected</option>
      ...
      <AnimatePresence>
        {paginatedApplicants.map((item, index) => {
          const applicant = item?.applicant;
          const currentStatus = updatedStatus[item._id] || item.status;
          const disabled =
          ...
          return (
            <motion.div
              ...
              whileHover={{ y: -2 }}
              className="
                relative bg-white border rounded-2xl shadow-sm hover:shadow-lg
                transition-all p-3 sm:p-4 lg:p-5
              "
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                ...
                <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                  ...
                  <div
                    ...
                    className="
                      ...
                      cursor-pointer transition duration-300 hover:scale-110 shrink-0
                    "
                  >
                    {applicant?.profile?.profilePhoto ? (
                      <img
                      ...
                      <div className="h-full w-full flex items-center justify-center font-semibold text-gray-500 text-sm">
                        {applicant?.fullname?.charAt(0)}
                      </div>
                    )}
                  </div>

                  {/* Text — add right padding so it doesn't collide with badge */}
                ...
                {/* RIGHT — Resume + Accept / Reject buttons */}
                  ...
                  {applicant?.profile?.resume && (
                    <a href={applicant.profile.resume} target="_blank" rel="noopener noreferrer">
                      <Button
                        size="sm"
                        variant="outline"
                        className="rounded-xl text-xs sm:text-sm h-8 sm:h-9 px-2.5 sm:px-3"
                      >
        ...
        {selectedApplicant && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1 }}
              exit={{ scale: 0.85 }}
              className="bg-white rounded-2xl w-full max-w-sm sm:w-[360px] p-5 sm:p-6 relative shadow-xl"
            >
              <X
                className="absolute right-4 top-4 cursor-pointer text-gray-500 hover:text-black transition w-4 h-4 sm:w-5 sm:h-5"
                onClick={() => setSelectedApplicant(null)}
              />
